...
public class ImageUtils {
     ...
     * 质量压缩
     */
    public static byte[] compressAndGetImageByte(Bitmap image) {
        if (image == null);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        Bitmap.CompressFormat Type = picType == 0 ? Bitmap.CompressFormat.PNG : Bitmap.CompressFormat.JPEG;
        //image.compress(Bitmap.CompressFormat.JPEG, 100, baos);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
        image.compress(Type, 90, baos);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
        int options = 100;
        while (baos.toByteArray().length / 1024 > 100) {  //循环判断如果压缩后图片是否大于120kb,大于继续压缩
    ...
    public static Bitmap getBitMBitmap(String urlpath) {
     /*   Bitmap map = null;
        ...
        Bitmap bm = null;
        try {
            java.net.URL iconUrl = new java.net.URL(urlpath);
            java.net.URLConnection conn = iconUrl.openConnection();
     ...
     * 通过图片url生成Bitmap byte[]
     *
     * @param urlpath
     * @return Bitmap
     * 根据图片url获取图片byte[]
     */
    public static byte[] getByteFromUrl(String urlpath) {
        Bitmap map = null;
        ...
        try {
            ...
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
//            map.compress(Bitmap.CompressFormat.PNG, 50, baos);
     ...
     * 根据url获取布局背景的对象
     */
    public static android.graphics.drawable.Drawable getDrawable(String urlpath) {
        ...
        try {
            ...
            java.io.InputStream in;
            in = conn.getInputStream();
            drawable = android.graphics.drawable.Drawable.createFromStream(in, "background.jpg");
        } catch (IOException e) {
            e.printStackTrace();
        }
        return drawable;
